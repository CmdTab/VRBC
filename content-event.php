...
<article id="post-<?php the_ID(); ?>" <?php post_class('single-event'); ?>>
	...
	<footer class="entry-meta">
		<?php
			...
			/* translators: used between list items, there is a space after the comma */
			$tag_list = get_the_tag_list( '', __( ', ', 'church' ) );
			...
			if ( ! church_categorized_blog() ) {
				...
				if ( '' != $tag_list ) {
					$meta_text = __( 'This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'church' );
				} else {
					$meta_text = __( 'This entry was posted in %1$s. Bookmark the <a href="%3$s" rel="bookmark">permalink</a>.', 'church' );
				}
